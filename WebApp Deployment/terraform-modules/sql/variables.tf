...
variable "sql_elastic_pool_name" {
  ...
  validation {
    condition     = length(var.sql_elastic_pool_name) < 63 && can(regex("^[a-z0-9-]*$", lower(var.sql_elastic_pool_name)))
    error_message = " The SQL Elastic Pool must be valid name, name can contain only lowercase letters, numbers, and '-', but can't start or end with '-' or have more than 63 characters."
  }
}
...
variable "sql_server_name" {
  ...
  validation {
    condition     = length(var.sql_server_name) < 63 && can(regex("^[a-z0-9-]*$", lower(var.sql_server_name)))
    error_message = " The SQLServervName must be valid name, name can contain only lowercase letters, numbers, and '-', but can't start or end with '-' or have more than 63 characters."
  }
}
